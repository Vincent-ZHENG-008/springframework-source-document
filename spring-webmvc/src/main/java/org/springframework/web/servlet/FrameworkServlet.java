...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected final void initServletBean() throws ServletException {
		...
		try {
			// 创建 WebApplicationContext，并初始化上下文信息
	...
	protected WebApplicationContext initWebApplicationContext() {
		// 构建 WebApplicationContext，传入 ServletContext
		// 从 ServletContext 中获得父容器 WebApplicationContext
		WebApplicationContext rootContext = WebApplicationContextUtils.getWebApplicationContext(getServletContext());
		// 声明子容器
		WebApplicationContext wac = null;

		// 如果 this.webApplicationContext 已存在，则将其设置为子容器(child ApplicationContext Container)
		// 建立父子容器之间的关系
		...
		// 从 ServletContext 中找 Web 容器的引用是否存在，并创建好默认的IOC容器
		if (wac == null) {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// 如果请求方式是 `PATCH` 或者无法解析
		if (httpMethod == HttpMethod.PATCH || httpMethod == null) {
			processRequest(request, response);
		} else {
			// 走原生请求方式装饰处理
			// 原生方法还会对具体的请求方式做处理，应该交还原生处理完后
	...
	protected final void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 重写子类方法，统一交给子类 ProcessRequest 统一处理
		processRequest(request, response);
	}
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// 构建国际化 Thread Locale Context
		LocaleContext previousLocaleContext = LocaleContextHolder.getLocaleContext();
		LocaleContext localeContext = buildLocaleContext(request);

		// 构建 ThreadLocal Request Attributes Context
		RequestAttributes previousAttributes = RequestContextHolder.getRequestAttributes();
		ServletRequestAttributes requestAttributes = buildRequestAttributes(request, response, previousAttributes);

		// 如果使用 Servlet Async 异步处理，则在上下文存储标识符
		WebAsyncManager asyncManager = WebAsyncUtils.getAsyncManager(request);
		// 异步处理器注册监听回调的拦截器
		asyncManager.registerCallableInterceptor(FrameworkServlet.class.getName(), new RequestBindingInterceptor());

		// 设置 Locale 和 RequestAttributes 到 ThreadLocal
		...
		try {
			...
			// 清除 Locale 和 RequestAttributes 在 ThreadLocal 上下文
