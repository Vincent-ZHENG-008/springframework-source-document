...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void doService(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// 打印 HttpServletRequest Trace 日志
		logRequest(request);

		// Keep a snapshot of the request attributes in case of an include,
		// to be able to restore the original attributes after the include.
		Map<String, Object> attributesSnapshot = null;
		// 判断是否内部 HttpServletRequest 请求
		...
		// 设置 WebApplicationContext 容器
		request.setAttribute(WEB_APPLICATION_CONTEXT_ATTRIBUTE, getWebApplicationContext());
		// 设置 LocaleResolver 国际化处理器
		request.setAttribute(LOCALE_RESOLVER_ATTRIBUTE, this.localeResolver);
		// 设置 ThemeResolver 模版处理器
		request.setAttribute(THEME_RESOLVER_ATTRIBUTE, this.themeResolver);
		// 设置 ThemeSource 模版数据源
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				// 判断是否 `multipart/form-data` 或 `multipart/`
				// 如果是，则解析请求参数解析到 MultiValueMap<String, MultipartFile>
				processedRequest = checkMultipart(request);
				// multipart请求 - 标志位
				multipartRequestParsed = (processedRequest != request);

				// Determine handler for the current request.
				// 1、尝试通过 HandlerMappings 获取 HandlerExecutionChain 执行链，也就是控制器的方法
				mappedHandler = getHandler(processedRequest);
				// 如果 handlerMapping 无法获取
				...
				// 2、根据 mappedHandler 寻找相应的 HandlerAdapter
				...
				// 3、执行 HandlerInterceptor(拦截器) 的前置方法
				...
				// 5、执行 HandlerInterceptor(拦截器) 的后置方法
			...
			// DispatcherServlet.doService 后置处理
			// 比如异常处理
	...
	protected HttpServletRequest checkMultipart(HttpServletRequest request) throws MultipartException {
		// 如果 Http Content-Type 为 `multipart/form-data` 或 `multipart/`
		if (this.multipartResolver != null && this.multipartResolver.isMultipart(request)) {
			if (WebUtils.getNativeRequest(request, MultipartHttpServletRequest.class) != null) {
				...
				try {
					// 通过装饰着模式包桩 HttpServletRequest，并将请求参数解析到 MultiValueMap<String, MultipartFile>
	...
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		if (this.handlerMappings != null) {
			// 遍历 HandlerMappings，通过 getHandler 匹配
			// HandlerMapping：
			// - RequestMappingHandlerMapping：通过 HandlerMethod 关联查找
			// - BeanNameUrlHandlerMapping
			// - RouterFunctionMapping
			// - SimpleUrlHandlerMapping
			// - WelcomePageHandlerMapping
			for (HandlerMapping mapping : this.handlerMappings) {
				// 如果匹配成功，则返回 HandlerExecutionChain，则进行迭代中断并返回
	...
	protected HandlerAdapter getHandlerAdapter(Object handler) throws ServletException {
		...
		if (this.handlerAdapters != null) {
			// 遍历 HandlerAdapter，查找匹配 HandlerExecutionChain 的并返回
